...
public class PathFinding {
     ...
     * @param allowDiagonals Pass true if you want 8 directional pathfinding, false
     *                       for 4 direcitonal
     * @return
     */
    public static List<Point_> findPath(Grid grid, Point_ startPos, Point_ targetPos, boolean allowDiagonals) {
        // Find path
        List<Node> pathInNodes = findPathNodes(grid, startPos, targetPos, allowDiagonals);

        // Convert to a list of Point_s and return
        List<Point_> pathInPoint_s = new ArrayList<Point_>();

        if (pathInNodes != null)
            for (Node node : pathInNodes)
                pathInPoint_s.add(new Point_(node.x, node.y));

        return pathInPoint_s;
    }
     ...
     */
    private static List<Node> findPathNodes(Grid grid, Point_ startPos, Point_ targetPos, boolean allowDiagonals) {
        ...
        while (openSet.size() > 0) {
            ...
            for (Node neighbour : neighbours) {
                ...
                int newMovementCostToNeighbour = currentNode.gCost
                        + getDistance(currentNode, neighbour) * (int) (10.0f * neighbour.price);
